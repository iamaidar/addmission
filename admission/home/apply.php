    ...
    if(isset($_SESSION['email'])){      
        ...
        if(isset($_POST['applyCourse'])){
            ...
            if(!$eResult->num_rows > 0) {
                ...
                $id = $details[0];      
                $query = $conn->prepare("SELECT id,coursename from selected_courses where id=?");            
                $query->bindValue(1, $id );
                $query->execute();

                if($query->rowCount() > 0 ) {
                    // Если курс уже выбран, выводим сообщение об ошибке
                    $msg = "<p style='text-align:center; color:red;'>You have already selected a course. You can choose only one course.</p>";
                } else {
                            ...
                            <form action="apply.php" method="post">
                                <div class="col-sm-12 form-group">
                                    ...
                                    <?php foreach($users as $user): ?>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="selectedCourse" value="<?= $user['coursename']; ?>">
                                                <?= $user['coursename']; ?>
                                            </label>
                                        </div>
                                    <?php endforeach; ?>
