			...
			<div class="container">
				...
				<div class="col-lg-12 well">
					<div class="row">
						<div class="col-sm-12">
							<div class="row">
								<div style="padding: 0 30px;" class="col-sm-6">
									<div class="row">
									...
									<form style="padding: 0; margin-top: 70px; width: 100%; display: flex; align-items: center;" method="POST">
										<div style="margin: 0; display: flex; align-items: center; width: 100%;" class="col-sm-6 form-group">
											<label for="exam_grade ">Exam Grade</label>
											<!-- <input type="number" class="form-control" id="exam_grade" name="exam_grade"> -->
											<input type="number" class="form-control" id="exam_grade" name="exam_grade" min="0" max="100">
										</div>
										<div style="margin: 0; display: flex; align-items: center; width: 100%;" class="col-sm-6 form-group">
											<label>Interview Grade</label>
											<!-- <input type="number" class="form-control" id="interview_grade" name="interview_grade"> -->
											<input type="number" class="form-control" id="interview_grade" name="interview_grade" min="0" max="100">
										</div>
										<div style="display: flex; align-self: center;" class="text-center"> 
											<button action="" type="submit" class="btn btn-primary">Submit</button> <!-- Добавляем класс btn и btn-primary для стилизации кнопки -->
										</div>
									</form>
								</div>
								<div style="display: flex; flex-direction: column; align-items: flex-end;" class="col-sm-6 form-group">
									<label>CV</label>
...
<?php 
    ...
    if (isset($_POST["exam_grade"]) && isset($_POST["interview_grade"])) {
        // Получение данных из формы
        $exam_grade = $_POST["exam_grade"];
        $interview_grade = $_POST["interview_grade"];

        // Подготавливаем SQL-запрос для вставки данных в таблицу grades
        ...
        // Выполняем запрос
        if ($stmt->execute()) {
            // После успешного добавления данных, перенаправляем пользователя
            // header("Location: http://localhost/admission/admin/grades.php");
			echo "<script>alert('Данные успешно сохранены.');</script>"; // Выводим алерт после успешного сохранения
            echo "<script>window.location = 'http://localhost/admission/admin/grades.php';</script>";
            exit; // Важно использовать exit после header, чтобы предотвратить выполнение остального кода
        } else {
                            ...
                            <div style="width: 100%;" class="row">
                                <div style="width: 75%; padding: 0 30px;" class="col-sm-6">
                                    ...
                                    <form style="padding: 0; margin-top: 70px; width: 100%; display: flex; align-items: center;" method="POST">
                                        ...
                                        <div style="margin-right: 10px; display: flex; align-items: center;">
                                            <button action="" type="submit" class="btn btn-primary" name="statusSubmit">Update Status</button>
                                        </div>
                                    </form>
                                </div>
        ...
        if ($exam_grade !== null && $interview_grade !== null) {
            ...
            if ($stmt->execute()) {
                ...
                if ($updateStmt->execute()) {
                    echo "<script>alert('Данные успешно сохранены.');</script>"; // Выводим алерт после успешного сохранения
                    echo "<script>window.location.href = window.location.href;</script>"; // Перезагружаем текущую страницу
                    exit;
                } else {
                    echo "Ошибка при обновлении статуса: " . $mysqli->error;
                }
            } else {
                echo "Ошибка при добавлении данных: " . $mysqli->error;
            }
            ...
            $updateStmt->bind_param("si", $approved, $sid);
            if ($updateStmt->execute()) {
                echo "<script>alert('Статус успешно обновлен.');</script>"; // Выводим алерт после успешного обновления
                echo "<script>window.location.href = window.location.href;</script>"; // Перезагружаем текущую страницу
                exit;
            } else {
                echo "Ошибка при обновлении статуса: " . $mysqli->error;
